...
--- ```sh
--- julia --project=~/.julia/environments/nvim -e 'using Pkg; Pkg.add("LanguageServer")'
--- ```
--- where `~/.julia/environments/nvim` is the location where
--- the default configuration expects LanguageServer.jl to be installed.
---
--- To update an existing install, use the following command:
--- ```sh
--- julia --project=~/.julia/environments/nvim -e 'using Pkg; Pkg.update()'
--- ```
---
...
local function activate_env(path)
  ...
  if path then
    ...
    vim.list_extend(
      environments, vim.fs.find(root_files, { type = "file", upward = true, limit = math.huge })
    )
...
local cmd = {
  ...
  [[
    # Load LanguageServer.jl: attempt to load from ~/.julia/environments/nvim
